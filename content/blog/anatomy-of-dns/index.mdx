...
We see that there is some text and just a bunch of different hex numbers, so this is the time to bring up the rfc spec. If we look into the _message_ section of the rfc, they have a image representing a query and response.

So all messages borth requests and responses have the following structure.
...
The header and question section is always included in both the request and response. But we are going to start with breaking down the header.

## The DNS Header

An image usually says a 1000 words so we are going to extract an image from the rfc once again.
