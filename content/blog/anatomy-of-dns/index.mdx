...
import InfoBox from "../../../src/elements/boxes/WarningBox";
...
myself really bad at everything related to networking. So when i find good blog
posts about things im weak they get stuck in my head.

It was blog post about how you would go about to write a DNS client. I just
glanced through it without looking at any implementations, I then decided to
look up the RFC and suddenly had this feeling that... i could write one of
those.

So in some blog posts i will write about all the things i learnt during the
development and they will both be DNS related, but also coding related, and how
i solved ceratin problems.
...
servers that you can query by a web address and get an ip-address back instead.
So think of DNS-servers as sort of old school phonebooks.

The original DNS specification is defined in the rfc 1035, and we are going to
reference that throughout this post.
...
One of the things we are going to see later is that the Heaader and the Question
is always going to be in the message and the repsonses, but depending on what
respons we get back the last three sections of the message will differ.

## The DNS Header

So we are going to start out with the header, and an image usually says more
than a 1000 words so we are going to extract an image (once again) rfc that
shows the overall structure of the header section.
...
Our first 2 bytes have the hex number `98ad`, which converted to decmial is
...
an authority for the domain name we are aasking about. Basically it denotes
if the name server we asked has _responsabillity_ over the domain.
...
usercases when you dont want to do recursive lookups. 
...
So our counters show us that there is one _question_ and one _additional record_ in our question.

<InfoBox>
ARCount is here set to 1 indicating that there is an _additional record_ included. DNS has pretty much _outgrown_ its header size and there was a need to be able to include more meta information in messages so DNS was extended in [rfc 6891](https://datatracker.ietf.org/doc/html/rfc6891) to include _psuedo records_ that can be used to include additional meta information in both requests and responses.
</InfoBox>


## DNS Question section

Lastly in a standard DNS message is the actual query. THis section contains the doman name we are asking for and and the definition of what type of _resource records_ we want.
...
```
# format:
<length><label><length><label><Nullbyte>
...
<InfoBox>
There are some aize limits defined in the RFC. For instance we are only allowed to have ASCII characters in names. Another restriction include a length limit of 63 octets or less, which means that is the length limit for a label in a domain name. This limit is actually defined because of how compression in DNS messages is implemented (the most significant bit is used, compression in DNS will be talked about in a different blog post). Additional limits that needs to be considered is 255 for a single domain name and 512 which is the maximum size of UDP messages as a whole.
</InfoBox>
...
might be a seperate post about it with a code example of how to solve the
